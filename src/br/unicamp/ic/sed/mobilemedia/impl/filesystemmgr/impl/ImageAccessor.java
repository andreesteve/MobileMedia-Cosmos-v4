...
import br.unicamp.ic.sed.mobilemedia.impl.filesystemmgr.spec.dt.ImageData;
import br.unicamp.ic.sed.mobilemedia.impl.filesystemmgr.spec.excep.ImageNotFoundException;
import br.unicamp.ic.sed.mobilemedia.impl.filesystemmgr.spec.excep.ImagePathNotValidException;
import br.unicamp.ic.sed.mobilemedia.impl.filesystemmgr.spec.excep.InvalidImageDataException;
import br.unicamp.ic.sed.mobilemedia.impl.filesystemmgr.spec.excep.InvalidImageFormatException;
 ...
 class ImageAccessor {
	...
	protected void addImageData(String photoname, IImageData imageData, String albumname) throws InvalidImageDataException, PersistenceMechanismException {
		try {
			...
			throw new PersistenceMechanismException("Impossible to add image data to record store.");
		}finally{
			try {
...
//				System.out.println("<* ImageAccessor.addImageData *> path = "+path);
//				System.out.println("<* ImageAccessor.addImageData *> albumname = "+albumname);
		try {
			imageRS = RecordStore
	...
	protected void createNewPhotoAlbum(String albumName) throws PersistenceMechanismException, InvalidPhotoAlbumNameException {
		...
		if (albumName.equals("")){
			throw new InvalidPhotoAlbumNameException("The new album name is invalid!");

		}
		
		//check if there is an album with the same name
		String[] albumNames  = this.getAlbumNames();
		System.out.println("[ImageAccessor] albumNames = "+albumNames);
		for (int i = 0; i < albumNames.length; i++) {
			if (albumNames[i].equals(albumName))
				throw new InvalidPhotoAlbumNameException("There is already a album with this name!");
		}

		try {
			newAlbumRS = RecordStore.openRecordStore(ALBUM_LABEL + albumName,
					true);
			System.out.println("[ImageAccessor] newAlbumRS = "+newAlbumRS);
			newAlbumInfoRS = RecordStore.openRecordStore(
					INFO_LABEL + albumName, true);
			System.out.println("[ImageAccessor] newAlbumInfoRS = "+newAlbumInfoRS);
	...
	private AlbumData getAlbumData() {
		System.out.println("[ImageAccessor] getAlbumData()");
		return albumData;
	}
	...
	private void setAlbumNames(String[] albumNames) {
		if( albumNames != null){
			...
			for(int i = 0;i<albumNames.length;i++){
				System.out.println("album["+i+"]="+albumNames[i]);
			}
		}
	 ...
	 * @throws NullAlbumDataReference 
	 */
	protected ImageData getImageInfo(String imageName) throws ImageNotFoundException, NullAlbumDataReference {
		AlbumData albumData = this.getAlbumData();
		if (albumData == null)
			throw new NullAlbumDataReference("Null reference to the Album data");

		ImageData ii = (ImageData) albumData.getImageInfoTable().get(imageName);
	 ...
	 * @throws PersistenceMechanismException 
	 */
	private byte[] loadImageBytesFromRMS(String recordName, String imageName,
